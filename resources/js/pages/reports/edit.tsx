...
import { index, update } from '@/routes/reports';
...
export default function EditReport() {
    const { auth, report } = usePage<EditPageProps>().props;
    const reportDate = new Date(`${report.report_date}T00:00:00`);
    const currentMonth = report.month_year;
    const currentYear = reportDate.getFullYear();
    const currentMonthNumber = reportDate.getMonth() + 1;
    ...
    // Update days in selected month; keep existing rows.
    useEffect(() => {
    ...
    const handleMonthChange = (month: string) => {
        ...
        const firstDayDate = new Date(selectedYear, monthNum - 1, 1);
        setData('report_date', toLocalDateInput(firstDayDate));
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(update.url({ report: report.id }));
    };
        ...
        <AppLayout>
            ...
            <div className="space-y-6">
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Edit Laporan</h1>
                        <p className="text-gray-600 mt-1">
                            Perbarui data laporan shift bulanan
                        </p>
                    </div>
                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(value) => {
                                            ...
                                            const monthNames = ['JANUARY', 'FEBRUARY', 'MARCH', 'APRIL', 'MAY', 'JUNE',
                                                'JULY', 'AUGUST', 'SEPTEMBER', 'OCTOBER', 'NOVEMBER', 'DECEMBER'];
                                            setData('month_year', `${monthNames[selectedMonth - 1]} ${year}`);
                                            const firstDayDate = new Date(year, selectedMonth - 1, 1);
                                            setData('report_date', toLocalDateInput(firstDayDate));
                                        }}
                                    >
                                        <SelectTrigger>
